...
export const Myfiles = () => {
  ...
  const fetchFolders = async (userId) => {
    try {
      ...
      const ownedRes = await axios.get(
        `http://15.235.147.39:3000/api/folders/user/${userId}`,
        {
      ...
      const sharedRes = await axios.get(
        `http://15.235.147.39:3000/api/folders/shared`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const addFolder = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://15.235.147.39:3000/api/folders/create-folder',
        {
  ...
  const deleteFolder = async (folderId) => {
    try {
      ...
      await axios.delete(`http://15.235.147.39:3000/api/folders/${folderId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const ShareFolderModal = ({ isOpen, onClose, folderId, onShare }) => {
    ...
    const handleShare = async () => {
      try {
        ...
        const response = await axios.post(
          `http://15.235.147.39:3000/api/folders/${folderId}/share`,
          { email, permission },
          { headers: { Authorization: `Bearer ${token}` } }
        );
